...
contract GameToken is ERC721, ERC721Metadata, MinterRole, ERC721Burnable, AssetTransferInterface, InterledgerSenderInterface, InterledgerReceiverInterface {
     ...
     * @param title     Title of tokens
     * @param symbol    Symbol of tokens
     * @param minters   Allowed minters (in addition to creator of the contract, who is automatically
     *                  added as minter)
     */
    constructor (string memory title, string memory symbol, address[] memory minters) public ERC721Metadata(title, symbol) {
        for (uint256 i = 0; i < minters.length; i++) {
            addMinter(minters[i]); // addMinter will get stuck if the same minter has already been added!
        }
    }
    ...
    // Transitions:
        // 2 -> 1: Token owner,      transferOut function
        // 1 -> 2: Contract minters, interledgerAbort function
        // 1 -> 0: Contract minters, interledgerCommit function
        // 0 -> 2: Contract minters, interledgerReceive function
        // 0 -> 2: Contract minters, accept function
    ...
    function interledgerCommit(uint256 id) public onlyMinter _existingAsset(id) {

        require(assetMap[id].state == State.TransferOut, "GameToken: tokenId must be in TransferOut state");

        assetMap[id].state = State.NotHere;
        emit NotHere(msg.sender, assetMap[id].name, id);
    }
    ...
    // For KSI
    function interledgerCommit(uint256 id, bytes memory data) public onlyMinter _existingAsset(id){
    ...
    /// @notice Set the token to Here state, used to handle errors
    /// @dev The asset changes state TransferOut -> Here. Callable by the game authority
    /// @param id The token id
    /// @param reason The reason
    function interledgerAbort(uint256 id, uint256 reason) public onlyMinter _existingAsset(id) {

        require(assetMap[id].state == State.TransferOut, "GameToken: tokenId must be in TransferOut state");

        assetMap[id].state = State.Here;
        emit Here(msg.sender, assetMap[id].name, id);
    }
